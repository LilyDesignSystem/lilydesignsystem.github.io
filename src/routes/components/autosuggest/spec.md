# Autosuggest — Specification

Single source of truth for spec-driven development of the Autosuggest component. Consolidates the prior `plan.md` and `tasks.md`.

## Goal

Implement the Autosuggest component: a text input that proposes matching options as users type.
...
1. Render `<div>` as the root element with `class="autosuggest"`
...
- [ ] Renders `<div>` with class `autosuggest`
