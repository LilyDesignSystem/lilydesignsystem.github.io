...
const title = 'Northern Ireland Health And Care Number Input';
...
test.describe('component page: ' + slug, () => {
  ...
  test('document title contains the component name', async ({ page }) => {
    await expect(page).toHaveTitle(/Northern Ireland Health And Care Number Input\s+\u2014\s+Lily Design System/);
  });
